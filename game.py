...
def stop_all_timers():
    ...
    countdown_active = False
    voting_timer_active = False
    # اضافه کردن تاخیر برای اطمینان از توقف تایمرها
    time.sleep(0.2)
...
def reset_game_completely():
    ...
    # متوقف کردن تایمرها - این مهمترین بخش است
    stop_all_timers()
    ...
    vote_start_time = None
    eliminated_players = set()
    game_active = False  # این خط بسیار مهم است
    game_settings = {}
    users_with_roles = {}
    vote_records = []
    used_usernames = set()
...
def get_active_players_list():
    """دریافت لیست بازیکنان فعال برای نمایش"""
    active_players = []
    ...
    if not active_players:
        return "هیچ بازیکن فعالی وجود ندارد"
...
def send_vote_results_to_all_players(final_results, eliminated_player=None, eliminated_role=None):
    """ارسال نتایج رای‌گیری به همه بازیکنان (فعال و حذف شده)"""
    citizen_count, mafia_count = get_role_counts()
    ...
    # ارسال به همه بازیکنان (فعال و حذف شده)
    for user_id, state in user_states.items():
        username = user_states.get(f"{user_id}_username")
        if username and state != 'game_ended':
            if state == 'eliminated':
                # برای بازیکنان حذف شده فقط نتایج را بفرست
                bot.send_message(user_id, 
                               f"📊 **نتایج رای‌گیری دور جدید:**\n\n"
                               f"{final_results}\n"
                               f"👥 آمار فعلی:\n"
                               f"🏘️ شهروند: **{citizen_count} نفر**\n"
                               f"🔪 مافیا: **{mafia_count} نفر**\n"
                               f"🎮 کل بازیکنان فعال: **{citizen_count + mafia_count} نفر**")
            elif state == 'voting' or state == 'player_confirmed':
                # برای بازیکنان فعال پیام کامل را بفرست
                active_players_list = get_active_players_list()
                if eliminated_player:
                    full_message = (
                        f"🎯 **دور جدید شروع شد!**\n\n"
                        f"🔴 بازیکن حذف شده: **{eliminated_player}** ({eliminated_role})\n\n"
...
def end_voting():
    ...
    # ابتدا تایمرها را متوقف کنیم
    stop_all_timers()
    
    # اگر بازی غیرفعال است، کاری نکنیم
    if not game_active:
        return None
    ...
    if vote_counts:
        ...
        if len(players_with_max_votes) > 1:
            ...
            # ارسال نتایج به همه بازیکنان
            send_vote_results_to_all_players(final_results)
            
            # قبل از شروع دور جدید، بررسی کنیم که بازی هنوز فعال است
            if game_active:
            ...
            if winner:
                # قبل از پایان بازی، نتایج آخرین رای‌گیری را نمایش بده
                end_game_with_final_vote_results(winner, final_results, eliminated_player, eliminated_role)
                return eliminated_player
            
            # قبل از شروع دور جدید، بررسی کنیم که بازی هنوز فعال است
            if game_active:
                # ارسال نتایج به همه بازیکنان
                send_vote_results_to_all_players(final_results, eliminated_player, eliminated_role)
                start_new_voting_round()
...
def end_game_with_final_vote_results(winner, final_results, eliminated_player, eliminated_role):
    """پایان بازی با نمایش نتایج آخرین رای‌گیری"""
    global game_active
    game_active = False
    ...
    # اضافه کردن نتایج آخرین رای‌گیری به پیام پایانی
    message += f"\n\n📊 **نتایج آخرین رای‌گیری:**\n{final_results}\n\n"
    message += f"📊 **آمار نهایی:**\n🏘️ شهروند: **{citizen_count} نفر**\n🔪 مافیا: **{mafia_count} نفر**\n🎮 کل بازیکنان فعال: **{total_players} نفر**"
    message += f"\n\n{roles_info}"
...
def voting_timer():
    """تایمر اصلی رای‌گیری - نسخه اصلاح شده"""
    global voting_timer_active
    voting_timer_active = True
    
    # زمان باقی مانده را محاسبه می‌کنیم
    remaining_time = vote_duration * 60
    
    while remaining_time > 0 and game_active and voting_timer_active:
        time.sleep(1)
        remaining_time -= 1
    
    # فقط اگر بازی هنوز فعال است و تایمر غیرفعال نشده، رای‌گیری را پایان بده
...
def end_voting_early():
    """پایان دادن زودهنگام به رای‌گیری - نسخه اصلاح شده"""
    # کمی تاخیر برای اطمینان از هماهنگی
    time.sleep(1)
    if game_active and voting_timer_active:
        eliminated_player = end_voting()
        if eliminated_player:
...
def get_vote_time_admin(message):
    ...
    if message.text.isdigit():
        ...
        # مخلوط کردن اسم‌ها برای توزیع عادلانه‌تر
        shuffled_usernames = usernames.copy()
        random.shuffle(shuffled_usernames)
        
        # توزیع نقش‌ها
        for username, role in zip(shuffled_usernames, roles):
