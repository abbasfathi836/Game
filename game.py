...
def get_vote_results():
    """نمایش نتایج رای‌گیری فعلی"""
    if not votes:
        return "هنوز هیچ رایی ثبت نشده است."
    
    vote_counts = Counter(votes.values())
    result = "📊 **نتایج رای‌گیری فعلی:**\n\n"
...
def start_countdown_timer():
    ...
    def countdown():
        ...
        while remaining_time > 0 and game_active:
            ...
            # ارسال به روز رسانی هر 30 ثانیه
...
def end_voting():
    ...
    if vote_counts:
        ...
        if len(players_with_max_votes) > 1:
            ...
            final_results = "📊 **نتایج نهایی رای‌گیری:**\n\n"
            for player, count in vote_counts.most_common():
                final_results += f"🎯 **{player}**: {count} رای\n"
...
def start_new_voting_round():
    ...
    for user_id, state in user_states.items():
        ...
        if username and username not in eliminated_players and state != 'eliminated' and state != 'game_ended':
            ...
            # ارسال پیام شروع رای‌گیری با نمایش وضعیت فعلی
            vote_results = get_vote_results()
            bot.send_message(user_id, 
                           f"🎯 **رای‌گیری شروع شد!**\n\n"
                           f"⏰ زمان رای‌گیری: **{vote_duration} دقیقه**\n\n"
...
def receive_vote(message):
    ...
    # ارسال نتایج به روز شده به کاربر
    vote_results = get_vote_results()
    bot.send_message(user_id, 
                   f"✅ **رای شما برای {target_username} ثبت شد!**\n\n"
                   f"{vote_results}")
