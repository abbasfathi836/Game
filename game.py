...
vote_duration = 3  # مقدار پیش‌فرض 3 دقیقه
...
def reset_game():
    ...
    global game_settings, users_with_roles, vote_records, used_usernames, vote_duration
    ...
    vote_records = []
    used_usernames = set()
    vote_duration = 3  # بازگشت به مقدار پیش‌فرض
...
def reset_game_completely():
    ...
    # فقط ادمین‌ها رو نگه دار، بقیه رو پاک کن
    admin_users = set(admins)
    user_states.clear()
    admins.clear()
    
    # ادمین‌های قبلی رو دوباره اضافه کن
...
def get_vote_time():
    return game_settings.get('vote_time', vote_duration)
...
def get_vote_results():
    ...
    # نمایش رای‌دهندگان - برعکس شده
    result += "\n🗳️ **رای‌دهندگان:**\n"
    for target in vote_counts.keys():
        voters = [voter for voter, voted_for in votes.items() if voted_for == target]
        if voters:
            result += f"• **{target}** ← {', '.join(voters)}\n"
    
    return result

def start_countdown_timer():
    """شروع شمارش معکوس برای همه بازیکنان"""
    ...
    def countdown():
        ...
        while remaining_time > 0 and game_active:
            ...
            # ارسال به روز رسانی هر 30 ثانیه یا 10 ثانیه پایانی
                ...
                for user_id, state in user_states.items():
                    ...
                    if username and username not in eliminated_players and state == 'voting':
                        try:
                            ...
                            # فقط تایمر نمایش داده شود، بدون نتایج آراء
                            bot.send_message(
                                user_id, 
                                f"{time_text}\n\n"
                            ...
                            print(f"خطا در ارسال شمارش معکوس: {e}")
...
def end_voting():
    global votes, eliminated_players, game_active, vote_start_time
...
def end_game(winner):
    global game_active, vote_start_time
    game_active = False
    vote_start_time = None
...
def start_new_voting_round():
    ...
    # بررسی و اطمینان از تنظیم بودن زمان رای‌گیری
    if vote_duration <= 0:
        global vote_duration
        vote_duration = 3  # مقدار پیش‌فرض
    ...
    for user_id, state in user_states.items():
        ...
        if username and username not in eliminated_players and state != 'eliminated' and state != 'game_ended':
            ...
            # ارسال پیام شروع رای‌گیری فقط با تایمر
            bot.send_message(user_id, 
                           f"🎯 **رای‌گیری شروع شد!**\n\n"
                           f"⏰ زمان رای‌گیری: **{vote_duration} دقیقه**\n\n"
                           f"💡 پس از پایان زمان، نتایج رای‌گیری اعلام خواهد شد.\n\n"
...
def check_username(message):
    ...
    if role:
        ...
        role_message = bot.send_message(user_id, 
                                      f"🎭 **نقش شما: {role}**\n\n"
                                      "⏳ این پیام در 3 ثانیه حذف خواهد شد...")
        
        # ایجاد ترد برای حذف پیام بعد از 1 ثانیه
...
def show_admin_panel(message):
    ...
    markup.add('🔄 ریست کامل بازی', '📊 وضعیت بازی')
    markup.add('⏹️ پایان بازی فعلی', '🏠 بازگشت به منوی اصلی')
    bot.send_message(chat_id, 
                    "👨‍💼 **پنل ادمین**\n\n"
...
def handle_admin_choice(message):
    ...
    if text == '➕ شروع بازی جدید':
        if game_active:
            bot.send_message(chat_id, "❌ **در حال حاضر یک بازی فعال وجود دارد!**\n\nلطفاً ابتدا بازی فعلی را پایان دهید.")
            show_admin_panel(message)
            return
        ...
        bot.send_message(chat_id, 
                        ...
                        parse_mode='Markdown')
    elif text == '🔄 ریست کامل بازی':
        reset_game_completely()
        bot.send_message(chat_id, 
                        "✅ **بازی با موفقیت ریست کامل شد!**\n\n"
                        "🗑️ تمام اطلاعات پاک شد.\n"
                        "👤 تمام کاربران حذف شدند.\n"
                        "⏰ زمان بازی صفر شد.\n\n"
    ...
    elif text == '⏹️ پایان بازی فعلی':
        if game_active:
            winner = check_game_end()
            if not winner:
                winner = "مافیا"  # اگر بازی ناتمام باشد، مافیا برنده اعلام شود
            end_game(winner)
            bot.send_message(chat_id, f"✅ **بازی فعلی به پایان رسید!**\n\n🏆 برنده: {winner}")
        else:
            bot.send_message(chat_id, "❌ **هیچ بازی فعالی وجود ندارد!**")
        show_admin_panel(message)
...
def show_game_status(message):
    ...
    if game_active:
        ...
        for username, role in users_with_roles.items():
            if username in eliminated_players:
                eliminated_list.append(f"{username} ({role})")
            else:
                active_players.append(f"{username} ({role})")
        ...
        status_msg = (
            ...
            f"🟢 **بازیکنان فعال ({len(active_players)})**:\n"
        )
        
        if active_players:
            status_msg += "\n".join([f"• {player}" for player in active_players])
        else:
            status_msg += "هیچ بازیکن فعالی وجود ندارد"
            
        status_msg += f"\n\n🔴 **بازیکنان حذف شده ({len(eliminated_list)})**:\n"
        
        if eliminated_list:
            status_msg += "\n".join([f"• {player}" for player in eliminated_list])
        else:
            status_msg += "هیچ بازیکن حذف شده‌ای وجود ندارد"
            
    else:
        status_msg = (
            "🔴 **هیچ بازی فعالی وجود ندارد**\n\n"
            f"📊 اطلاعات آخرین بازی:\n"
            f"👥 بازیکنان ثبت شده: **{len(users_with_roles)}**\n"
            f"⏰ زمان رای‌گیری: **{vote_duration} دقیقه**\n"
            f"🔴 بازیکنان حذف شده: **{len(eliminated_players)}**"
        )
...
def get_vote_time_admin(message):
    ...
    if message.text.isdigit():
        vote_time = int(message.text)
        
        # اعتبارسنجی زمان رای‌گیری
        if vote_time < 1:
            bot.send_message(user_id, "❌ زمان رای‌گیری باید حداقل 1 دقیقه باشد. لطفاً مجدداً وارد کنید:")
            return
        if vote_time > 60:
            bot.send_message(user_id, "❌ زمان رای‌گیری نمی‌تواند بیشتر از 60 دقیقه باشد. لطفاً مجدداً وارد کنید:")
            return
        ...
        game_settings['vote_time'] = vote_time
        
        response = ("✅ **بازی جدید ایجاد شد!**\n\n"
                   f"📊 تعداد بازیکنان: **{total_players}**\n"
                   f"🏘️ شهروند: **{citizen_count}**\n"
                   f"🔪 مافیا: **{mafia_count}**\n"
                   f"⏰ زمان رای‌گیری: **{vote_time} دقیقه**\n\n"
                   f"👤 بازیکنان ثبت شده:\n**" + "\n".join(usernames) + "**\n\n"
                   f"📢 بازیکنان می‌توانند با ارسال /start و انتخاب 'ورود به بازی' وارد شوند.")
