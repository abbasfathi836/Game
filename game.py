...
def end_voting():
    ...
    # 🔥 ایجاد پیام نتایج کامل برای نمایش به همه
    results_message = "🎯 **نتایج نهایی رای‌گیری این دور:**\n\n"
    
    # نمایش آرای هر بازیکن به ترتیب بیشترین رأی
    for player, count in vote_counts.most_common():
        voter_list = [voter for voter, target in votes.items() if target == player]
        voters_str = "، ".join(voter_list)
        results_message += f"👤 **{player}**: {count} رای\n"
        results_message += f"   ↳ از: {voters_str}\n\n"
    
    # نمایش بازیکنانی که رأی نداده‌اند
    non_voters = [player for player in all_players if player not in votes and player not in eliminated_players]
    if non_voters:
        results_message += f"🚫 **رای نداده‌اند**: {', '.join(non_voters)}\n\n"
    
    if vote_counts:
        max_votes = max(vote_counts.values())
        players_with_max_votes = [player for player, count in vote_counts.items() if count == max_votes]
        
        # 🔥 ارسال نتایج به همه بازیکنان فعال
        for user_id, state in user_states.items():
            username = user_states.get(f"{user_id}_username")
            if username and state != 'eliminated' and state != 'game_ended':
                try:
                    bot.send_message(user_id, results_message, parse_mode='Markdown')
        ...
        if len(players_with_max_votes) > 1:
            ...
            # ارسال پیام حذف با جزئیات
            elimination_message = (
                f"❌ **بازیکن حذف شده: {eliminated_player}**\n"
                f"🎭 نقش: {eliminated_role}\n"
                f"📊 با {max_votes} رای حذف شد\n\n"
                f"------------------------"
            )
            ...
            for user_id, state in user_states.items():
                ...
                if username and username != eliminated_player and state != 'eliminated' and state != 'game_ended':
                    try:
                        bot.send_message(user_id, elimination_message)
                    except:
                        pass
            
            # حذف بازیکن از لیست کاربران فعال
            for user_id, state in user_states.items():
...
def end_game(winner):
    ...
    # نمایش نقش همه بازیکنان در پایان بازی
    roles_message = "\n\n🎭 **نقش همه بازیکنان:**\n"
    for username, role in users_with_roles.items():
        status = "❌ حذف شده" if username in eliminated_players else "✅ فعال"
        roles_message += f"👤 {username}: {role} ({status})\n"
